  ...
  if ((buttonState != lastButtonState) && (buttonState == HIGH) && (recording == false)) {
    ...
    while (recording){  // While we are recording…
      if (newFile) { // If the new file was opened successfully…
            //newFile.print(analogRead(A0)/4096.0*3.3, 6); // Read off the potentiometer values and save to SD file.
            Serial.print(analogRead(A0)/4096.0*3.3, 6); // And again for serial monitor

            // Finish off each line ready for next iteration
            //newFile.println(","); // Print tab and new line seperator for SD file
            Serial.println(","); // Print comma and new line seperator for Serial Monitor csv
            delay(2);
    ...
    delay(1000); // Delay .5 seconds for stability 
// Reset variables
    buttonState = 0; 
    lastButtonState = buttonState;
    recording = false; 
